...
class FileSaver:
    ...
    def save_processed_image(self, image, filename):
        if self.results_dir and image:
            ...
            messagebox.showwarning("Warning", "Results directory is not set or image is missing")

    def save_image(self, image):
        if self.results_dir and image:
            file_path = filedialog.asksaveasfilename(title="Save Image", defaultextension=".png",
...
class FileManager:
    ...
    def process_all_images(self):
        ...
        if not self.file_saver.results_dir:
            messagebox.showwarning("Warning", "Please set a results directory first.")
            return
        ...
        for file_name in image_files:
            ...
            try:
                ...
                self.file_saver.save_processed_image(self.app.current_image, f"processed_{file_name}")

            except Exception as e:
                messagebox.showerror("Error", f"Failed to process {file_name}:\n{e}")
